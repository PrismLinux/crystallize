...
pub async fn check_internet_connection_async() -> bool {
  ...
  for endpoint in &endpoints {
    ...
    match ping_result {
      Ok(Ok(Ok(output))) => {
        // Outer Ok for tokio::timeout, middle Ok for spawn_blocking, inner Ok for output()
  ...
  let dns_result = timeout(
    ...
    tokio::task::spawn_blocking(|| Command::new("nslookup").arg("google.com").output()),
  )
  .await;
